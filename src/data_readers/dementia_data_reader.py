...
def get_data_generator(dataset: str, image_size=(224, 224), batch_size=32):
    ...
    train_generator = train_datagen.flow_from_directory(
        os.path.join("..", "data", "dementia", "dataset", dataset),
...
def read_data():
    print(
        len(
            os.listdir(
                os.path.join("..", "..", "data", "dementia", "dataset", "train")
            )
        )
    )
    print(
        len(
            os.listdir(
                os.path.join("..", "..", "data", "dementia", "dataset", "test")
            )
        )
    )
    print(
        len(
            os.listdir(
                os.path.join("..", "..", "data", "dementia", "dataset", "val")
            )
        )
    )
...
def prepare_data_for_class(class_name):
    files = os.listdir(
        os.path.join("..", "..", "data", "dementia", "data", class_name)
    )
...
def prepare_dataset():
    prepare_data_for_class("Mild_Demented")
    prepare_data_for_class("Moderate_Demented")
    prepare_data_for_class("Non_Demented")
    prepare_data_for_class("Very_Mild_Demented")
    pass
